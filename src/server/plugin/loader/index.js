...
internals.register = module.exports = (server, options, next) => {
    async.auto({
        ...
        server.log(['info', internals.register.attributes.pkg.name],
            'core plugin loaded successfully.');
...
internals.registerPreHandlers = (server, options, callback) => {
    ...
    _.map(loadFiles(options.pattern, options.glob), (file) => {
        ...
        server.log(['info', 'registerPreHandlers'],
            `registered preHandlers in file ${preHandlerName}`);
    });
...
internals.registerHandlers = (server, options, callback) => {
    ...
    _.map(loadFiles(options.pattern, options.glob), (file) => {
        ...
        const handler = new handlerFile(server);
